  ...
  leg_detector_msgs__msg__LegArray__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    leg_detector_msgs__msg__LegArray * data =
      (leg_detector_msgs__msg__LegArray *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!leg_detector_msgs__msg__LegArray__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          leg_detector_msgs__msg__LegArray__fini(&output->data[i]);
        }
        return false;
      }
    }
