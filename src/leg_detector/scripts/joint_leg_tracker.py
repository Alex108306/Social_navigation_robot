...
# Custom messages
from leg_detector_msgs.msg import Human, HumanArray, Leg, LegArray
...
sys.path.append('/home/hhd/anaconda3/envs/peopleros2/lib/python3.8/site-packages')
...
class KalmanMultiTrackerNode(Node):
    ...
    def __init__(self):
        ...
        # Get ROS params
        self.fixed_frame = self.get_parameter_or("fixed_frame","laser_frame")
    	...
    	# ROS publishers
        self.people_tracked_pub = self.create_publisher(HumanArray, "people_tracked", 300)
        self.people_detected_pub = self.create_publisher(HumanArray, "people_detected", 300)
        ...
        # print(PersonArray)
        # self.get_logger().info(PersonArray)
        # ROS subscribers 
    ...
    def detected_clusters_callback(self, detected_clusters_msg):
        ...
        for detect in non_leg_clusters:
            ...
            # self.get_logger().info('The value x: %d, y:%d' % (non_leg.position.x, non_leg.position.y))
            ...
            for person in self.objects_tracked:
                if person.is_person == True:
                    if self.publish_occluded or person.seen_in_current_scan: # Only publish people who have been seen in current scan, unless we want to publish occluded people
                        ...
                        # self.get_logger().info("people po:%d, %d, %d"%(new_person.id, new_person.pose.position.x, new_person.pose.position.y))
